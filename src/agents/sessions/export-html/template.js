(function () {
  ...
  const searchString = injectedParams ? injectedParams.content : window.location.search.slice(1);
    ...
    switch (name) {
      ...
      case "ls": {
        ...
        if (result) {
          const output = getResultText().trim();
          if (output) {
            html += formatExpandableOutput(output, 20);
          }
        }
        break;
      }
  ...
  function getSidebarBounds() {
    ...
    const minWidth = Number.parseFloat(rootStyles.getPropertyValue("--sidebar-min-width")) || 240;
    const maxWidth = Number.parseFloat(rootStyles.getPropertyValue("--sidebar-max-width")) || 720;
